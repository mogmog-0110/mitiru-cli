...
# #56: vcpkg の applocal は pwsh を使い、その path を版固定の WindowsApps パスでキャッシュする。
# pwsh が WindowsApps 自動更新で版を上げると旧パスが消え、applocal が「指定されたパスが
# 見つかりません」で落ちて mitiru build/run が起動不能になる。engine の是正モジュールを
# project() (= vcpkg toolchain の pwsh find_program) より前に読み、版非依存 alias へ固定する。
# OPTIONAL: 旧 engine (モジュール未同梱) では無視される。
include("{{.EngineRoot}}/cmake/VcpkgPwshFix.cmake" OPTIONAL)
